                      ...
                      SingleChildScrollView(
                        child: Column(
                          ...
                          children: [
                            ...
                            _HelpSection(title: '1. Connect your Drives', text: 'Select Add account and choose Google Drive or Microsoft OneDrive. Approve access in your browser. Repeat for each account.'),
                            _HelpSection(title: kIsWeb ? 'Browser transfers and limits' : 'Windows transfers and limits', text: kIsWeb ? 'Web transfers and uploads use temporary browser memory, with a 32 MiB per-file limit, 10,000 items and 64 folder levels per batch. Keep this tab open until completion. Closing or reloading ends the job; automatic resume is unavailable. Microsoft sessions are memory-only: reconnect after reloading. Downloads also have a 32 MiB limit. Allow sign-in popups. Copy/Paste and internal drag-and-drop verify destination content with SHA-256 and an extra download. Move asks Yes/No only after verification; only unchanged OneDrive source files support conditional Recycle Bin cleanup. Google originals and folder containers remain. No keeps both copies. Interrupted jobs may leave destination copies. Browser or organization restrictions can block transfers; originals remain. Use Windows for larger files (up to 1 GiB per file). No fixed account count is imposed by FarooqDrive; provider policies, quotas and browser resources apply.' : 'Copy/Paste and internal drag-and-drop use a temporary disk file, sequential uploads and full SHA-256 destination verification. Transfer limit: 1 GiB per file, 10,000 items and 64 folder levels per batch. Keep enough free disk space for the largest file plus normal Windows needs. Transfer traffic uses your internet connection, including a second destination download for verification. Google-native same-account copies preserve their format and retain originals; between accounts, supported documents export to Office formats or PNG. Move asks Yes/No after verification. Only unchanged OneDrive source files support conditional Recycle Bin cleanup; Google source files and original folder containers remain. No keeps both copies. Interrupted uploads may leave destination copies; retries create new copies. Automatic resume after restarting is not available. Windows file-picker uploads stream through disk and verify the destination. Download-to-PC still uses memory; OneDrive download limit is 32 MiB. Account connections have no fixed app cap; service quotas, organization policies and device resources apply.'),
                            _HelpSection(title: 'Temporary storage', text: kIsWeb ? 'FarooqDrive stages file content in temporary browser memory; it does not create a Windows transfer-cache folder from the web app. The browser manages its own memory and downloads. Several buffers may be needed per file. File-list indexes, preferences and local activity can persist in this browser; file contents and Microsoft tokens are not stored in the saved index. Disconnect an account and clear site data on shared computers.' : 'Transfer cache: ${TransferSpool.cachePath}. Files are not encrypted by FarooqDrive in this folder. Completed or failed jobs remove their own cache files; an app crash may leave job folders here. Close FarooqDrive before manually removing leftover job folders. Transfers never delete original local files.'),
                        ...
                        child: Directionality(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              _HelpSection(title: 'عارضی جگہ اور حدود', text: kIsWeb ? 'ویب میں فی فائل 32 MiB کی حد ہے؛ نقل عارضی browser memory میں بنتی ہے۔ کام کے دوران ٹیب کھلا رکھیں۔ دوبارہ کھولنے پر Microsoft اکاؤنٹ دوبارہ منسلک کریں۔ Index اور ترجیحات browser میں محفوظ رہ سکتی ہیں، فائل کا مواد نہیں۔ بڑی فائل کے لیے Windows استعمال کریں۔ Move کی آخری تصدیق سے پہلے اصل فائل حذف نہیں ہوتی۔' : 'اس نسخے میں فی فائل 1 GiB، فی کام 10,000 اشیاء اور 64 فولڈر سطحوں کی حد ہے۔ سب سے بڑی فائل کے لیے ہارڈ ڈسک میں خالی جگہ رکھیں۔ FarooqDrive کی عارضی نقل encrypted نہیں ہے۔ کام کے بعد عارضی نقل مٹتی ہے؛ crash پر بچی ہوئی job folders ایپ بند کرکے ہٹائیں۔ دوبارہ شروع ہونے پر خودکار resume موجود نہیں۔ منزل کی تصدیق کے لیے فائل دوبارہ download ہوتی ہے، اس لیے انٹرنیٹ بھی استعمال ہوتا ہے۔ Accounts کی کوئی مقررہ app حد نہیں۔'),
                              ...
                              _HelpSection(title: '۵۔ گنجائش اور سرگرمی', text: 'گنجائش کے خانے Google یا Microsoft کی فراہم کردہ معلومات دکھاتے ہیں۔ تاریخ کا نشان اس آلے پر گزشتہ سات دنوں کی سرگرمی دکھاتا ہے۔'),
                              _HelpSection(title: 'رازداری اور حفاظت', text: 'فائلیں براہ راست آپ کے آلے اور منتخب Google Drive یا Microsoft OneDrive کے درمیان منتقل ہوتی ہیں۔ FarooqDrive کوئی درمیانی فائل ذخیرہ کرنے والا سرور استعمال نہیں کرتا۔'),
                            ],
                          ),
                        ),
                      ),
                ...
                IconButton(
                  ...
                  onPressed: () => showDialog<void>(context: context, builder: (context) => AlertDialog(
                    ...
                    content: SizedBox(width: 560, child: SingleChildScrollView(child: Column(
                      ...
                      const SelectableText(kIsWeb ? 'Web transfers use temporary browser memory and extra verification downloads. Limit: 32 MiB/file. Keep this tab open. See Help for privacy and limits.' : 'Cloud transfers use temporary disk space and extra verification downloads. Transfer limit: 1 GiB/file. See Help for cleanup and limits.',
